...
Namespace DBAccess
    Public Class LabDB
        ...
        Public Sub New(ByVal LabID As Integer)
            Using reader As ExecuteReaderResult = DataCommand.Create().Param(New With {.Action = "Select", LabID}).ExecuteReader("sselScheduler.dbo.procLabSelect")
                If reader.Read() Then
                    IsValid = True
        ...
        Public Function SelectByBuilding(ByVal BuildingID As Integer) As ExecuteReaderResult
            Return DataCommand.Create().Param(New With {.Action = "SelectByBuilding", BuildingID}).ExecuteReader("sselScheduler.dbo.procLabSelect")
        End Function

        Public Function SelectByBuilding(ByVal BuildingID As Integer, ByVal bDataTable As Boolean) As DataTable
            Return DataCommand.Create().Param(New With {.Action = "SelectByBuilding", BuildingID}).FillDataTable("sselScheduler.dbo.procLabSelect")
        End Function

        ' Returns all Labs
        Public Shared Function SelectAll() As DataTable
            Dim dt As DataTable = DataCommand.Create().MapSchema().Param(New With {.Action = "SelectAll"}).FillDataTable("sselScheduler.dbo.procLabSelect")
        ...
        Public Function SelectRooms() As ExecuteReaderResult
            Return DataCommand.Create().Param(New With {.Action = "All"}).ExecuteReader("dbo.Room_Select")
        End Function

        Public Function HasProcessTechs(ByVal LabID As Integer) As Boolean
            Dim count As Integer = DataCommand.Create().Param(New With {.Action = "HasProcessTechs", LabID}).ExecuteScalar(Of Integer)("sselScheduler.dbo.procLabSelect").Value
            Return count > 0
        End Function
